...
@implementation UIButton (LXLayoutExt)
    ...
    if (self.imageView.image && self.lx_layoutType != LXButtonLayoutTypeNone) {
        switch (self.lx_layoutType) {
            ...
            case LXButtonLayoutTypeImageTop:
                ...
                break;
            case LXButtonLayoutTypeImageRightDown:
                [self layoutSubviewsByTypeRightDown];
                break;
            default:
                break;
        }
...
- (void)setLx_imageYMargin:(CGFloat)lx_imageYMargin {
    if (self.lx_imageYMargin == lx_imageYMargin) {
        return;
    }
    objc_setAssociatedObject(self, @selector(lx_imageYMargin),[NSNumber numberWithFloat:lx_imageYMargin],OBJC_ASSOCIATION_COPY_NONATOMIC);
    [self setNeedsLayout];
}

- (CGFloat)lx_imageYMargin {
    NSNumber * result = objc_getAssociatedObject(self, @selector(lx_imageYMargin));
    return [result floatValue];
}
- (void)setLx_imageViewSize:(CGSize)lx_imageViewSize {
    if (CGSizeEqualToSize(self.lx_imageViewSize, lx_imageViewSize)) {
        return;
    }
    objc_setAssociatedObject(self, @selector(lx_imageViewSize),[NSValue valueWithCGSize:lx_imageViewSize],OBJC_ASSOCIATION_COPY_NONATOMIC);
    [self setNeedsLayout];
}

- (CGSize)lx_imageViewSize {
    NSValue * result = objc_getAssociatedObject(self, @selector(lx_imageViewSize));
...
- (void) layoutSubviewsByTypeLeft {
    ...
    if (!CGSizeEqualToSize(self.lx_imageViewSize, CGSizeZero)) {
        imageWidth = self.lx_imageViewSize.width;
        imageHeight = self.lx_imageViewSize.height;
    }
...
- (void) layoutSubviewsByTypeRightDown {
    ...
    CGFloat imageViewX = CGRectGetMaxX(self.titleLabel.frame) + self.lx_subMargin;
    CGFloat imageViewY = (CGRectGetHeight(self.frame) - imageHeight)+self.lx_imageYMargin;
    self.imageView.frame = CGRectMake(imageViewX, imageViewY, imageWidth, imageHeight);
